...
pub fn build_expression_node<'a>(pair: Pair<'a, Rule>) -> Result<ExpressionNode<'a>, AstBuilderError> {
    match pair.as_rule() {
        ...
        _ => Err(AstBuilderError(format!("Unknown expression rule: {:?}", pair.as_rule()))),
    }
}
