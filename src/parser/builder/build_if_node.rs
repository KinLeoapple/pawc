...
pub fn build_if_node<'a>(pair: Pair<'a, Rule>) -> Result<StatementNode<'a>, AstBuilderError> {
    ...
    let _if_kw = inner
        .next()
        .ok_or_else(|| AstBuilderError("if_statement: missing 'if' keyword".into()))?;
    // 主 if 条件
    let cond_pair = inner
        .next()
        .ok_or_else(|| AstBuilderError("if_statement: missing condition".into()))?;
    let cond = build_expression_node(cond_pair)?;

    // 主 if 块
    let then_body_pair = inner
        .next()
        .ok_or_else(|| AstBuilderError("if_statement: missing then body".into()))?;
                ...
                if let Some(next_pair) = inner.peek() {
                    if next_pair.as_rule() == Rule::KEYWORD_IF {
                        ...
                        inner.next(); // consume IF
                                      // 解析 else if 分支
                        let else_if_cond_pair = inner.next().ok_or_else(|| {
                            AstBuilderError("if_statement: missing else-if condition".into())
                        })?;
                        let else_if_cond = build_expression_node(else_if_cond_pair)?;
                        let else_if_body_pair = inner.next().ok_or_else(|| {
                            AstBuilderError("if_statement: missing else-if body".into())
                        })?;
                        ...
                        // 普通 else
                        let else_body_pair = inner.next().ok_or_else(|| {
                            AstBuilderError("if_statement: missing else body".into())
                        })?;
