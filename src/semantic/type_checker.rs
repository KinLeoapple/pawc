...
fn is_numeric(t: &PawType) -> bool {
    matches!(t, PawType::Int | PawType::Long | PawType::Float | PawType::Double)
}
...
impl TypeChecker {
    ...
    pub fn check_statements(&mut self, stmts: &[Statement]) -> Result<(), PawError> {
        // 阶段一：预注册函数签名
        for stmt in stmts {
            if let StatementKind::FunDecl { name, return_type, .. } = &stmt.kind {
                let ret_ty = return_type
                    .as_deref()
                    .map(PawType::from_str)
                    .unwrap_or(PawType::Void);
                self.scope
                    .define(name, ret_ty, 0, 0)
    ...
    pub fn check_statement(&mut self, stmt: &Statement) -> Result<(), PawError> {
        match &stmt.kind {
            StatementKind::Import { alias, .. } => {
                self.scope
                    .define(alias, PawType::Module, 0, 0)
                    .map_err(|_| PawError::DuplicateDefinition {
                        code:    "E2005",
                        name:    alias.clone(),
                        line:    0,
            ...
            StatementKind::Let { name, ty, value } => {
                ...
                if let Expr::LiteralNopaw = *value {
                    if matches!(expected, PawType::Optional(_)) {
                        self.scope
                            .define(name, expected, 0, 0)
                            .map_err(|_| PawError::DuplicateDefinition {
                                code:    "E2005",
                                name:    name.clone(),
                                line:    0,
                        ...
                        return Err(PawError::Type {
                            code:    "E3002",
                            message: format!("Cannot assign `nopaw` to {} in let '{}'", expected, name),
                            line:    0,
                            column:  0,
                            snippet: None,
                ...
                if expected != actual && expected != PawType::Any && !covariant {
                    return Err(PawError::Type {
                        code:    "E3003",
                        message: format!("Type mismatch in let '{}': expected {}, found {}", name, expected, actual),
                        line:    0,
                        column:  0,
                        snippet: None,
            ...
            StatementKind::Assign { name, value } => {
                let expected = self.scope.lookup(name).ok_or_else(|| PawError::UndefinedVariable {
                    code:    "E4001",
                    name:    name.clone(),
                    line:    0,
                    column:  0,
                ...
                if expected != actual && expected != PawType::Any && !covariant {
                    return Err(PawError::Type {
                        code:    "E3005",
                        message: format!("Type mismatch in assignment to '{}': expected {}, found {}", name, expected, actual),
                        line:    0,
                        column:  0,
                        snippet: None,
            ...
            StatementKind::Say(expr) => { self.check_expr(expr)?; }
            ...
            StatementKind::Return(opt) => { if let Some(e) = opt { self.check_expr(e)?; }}
            ...
            StatementKind::If { condition, body, else_branch } => {
                ...
                if let Some(else_stmt) = else_branch { child.check_statement(else_stmt)?; }
            }

            StatementKind::LoopForever(body) => {
                TypeChecker::with_parent(&self.scope).check_statements(body)?;
            }
            ...
            StatementKind::LoopRange { var, start, end, body } => {
                ...
                let mut child = TypeChecker::with_parent(&self.scope);
                child.scope.define(var, s.clone(), 0, 0).map_err(|_| PawError::DuplicateDefinition {
                    code:    "E2005",
                    name:    var.clone(),
                    line:    0,
                    column:  0,
            ...
            StatementKind::FunDecl { name, params, body, .. } => {
                let prev = self.current_fn.clone();
                self.current_fn = Some(name.clone());
                let mut sub = TypeChecker::with_parent(&self.scope);
                for p in params {
                    let pty = PawType::from_str(&p.ty);
                    sub.scope.define(&p.name, pty, 0, 0).map_err(|_| PawError::DuplicateDefinition {
                        code:    "E2005",
                        name:    p.name.clone(),
                        line:    0,
                        column:  0,
            ...
            StatementKind::Block(stmts) => {
                TypeChecker::with_parent(&self.scope).check_statements(stmts)?;
            }

            StatementKind::TryCatchFinally { body, err_name, handler, finally } => {
                let _ = TypeChecker::with_parent(&self.scope).check_statements(body);
                let mut csc = TypeChecker::with_parent(&self.scope);
                csc.scope.define(err_name, PawType::String, 0, 0).map_err(|_| PawError::DuplicateDefinition {
                    code:    "E2005",
                    name:    err_name.clone(),
                    line:    0,
                    column:  0,
                ...
                TypeChecker::with_parent(&self.scope).check_statements(finally)?;
            }
        }
        Ok(())
    }
    ...
    pub fn check_expr(&mut self, expr: &Expr) -> Result<PawType, PawError> {
        if let Expr::LiteralNopaw = expr {
            return Ok(PawType::Optional(Box::new(PawType::Any)));
        }
        ...
        match expr {
            ...
            Expr::UnaryOp { op, expr } => {
                ...
                match op.as_str() {
                    "-" if matches!(t, PawType::Int | PawType::Long | PawType::Float) => Ok(t),
                    "!" => Ok(PawType::Bool),
                    _ => Err(PawError::Type {
                        code:    "E3013",
            ...
            Expr::BinaryOp { left, op, right } => {
                ...
                let numeric = |x: &PawType| {
                    matches!(x, PawType::Int | PawType::Long | PawType::Float | PawType::Double)
                };
            ...
            Expr::Call { name, args } => {
                ...
                self.scope.lookup(name).ok_or_else(|| PawError::UndefinedVariable {
                    code:    "E4001",
                    name:    name.clone(),
                    line:    0,
                    column:  0,
